...
export function DemoChrome() {
  const { resetDemo, extras, lastGift } = useDemo();
    ...
    <>
      <div className="fixed bottom-4 left-4 z-50 flex max-w-[min(92vw,360px)] flex-col gap-2">
        <div className="rounded-2xl border border-line bg-[color:var(--paper)] px-3 py-2 text-[11px] leading-snug text-ink-soft shadow-[var(--shadow)]">
          <span className="font-medium text-ink">Prototype.</span> Teachers, donors, and fulfillment history are seeded demo records. Legislation pages are sourced from azleg.gov and labeled separately.
        </div>
      </div>
      <div className="fixed bottom-4 right-4 z-50 flex flex-col items-end gap-2">
        {count > 0 && (
          <p className="rounded-full bg-juniper px-3 py-1 text-[11px] text-[color:var(--paper)]">
            Demo gifts applied: {count}
            {lastGift ? " · last action saved" : ""}
          </p>
        )}
        <button
          onClick={resetDemo}
